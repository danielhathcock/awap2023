...
class BotPlayer(Player):
    ...
    def play_turn(self, game_state: GameState) -> None:
        ...
        if ginfo.turn == 1:
            tile1 = ally_tiles[0]
            game_state.spawn_robot(RobotType.TERRAFORMER, tile1.row, tile1.col)

        if ginfo.turn == 2:
            rname, rob = list(robots.items())[0]
            a_tile = game_state.get_map()[rob.row][rob.col+1]
            print(a_tile.robot)
            ...
            a_tile = game_state.get_map()[rob.row][rob.col+1]
            print(a_tile.robot)
            # game_state.spawn_robot(RobotType.EXPLORER, rob.row, rob.col)
            game_state.robot_action(rname)

        if ginfo.turn == 6:
            r1, rob1 = list(robots.items())[0]
            game_state.move_robot(r1, Direction.LEFT)
