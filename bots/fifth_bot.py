...
class BotPlayer(Player):
    ...
    def get_explorer_spwan_location(self, threshold):
        ...
        for row in range(self.height):
            for col in range(self.width):
                if self.tiles[row][col] == None: continue
                if self.tiles[row][col].robot != None: continue
    ...
    def get_terraformable_tiles(self, row, col) -> int:
        ...
        for d in Direction:
            tile_info = self.get_tile_info(row + d.value[0], col + d.value[1])
            if tile_info.state != TileState.ILLEGAL and tile_info.state != TileState.IMPASSABLE:
